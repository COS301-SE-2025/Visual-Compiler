   ...
   const faqs = [
    {
      ...
      answer: 'Simply click on one of the blocks in the "Blocks" panel on the left. This will add a new node of that type to the workspace.'
    },
    {
      question: 'How do I move a node?',
      answer: 'For the smoothest experience, click a node once to select it (it will be highlighted), and then click and drag it to a new position.'
    },
    {
      question: 'How do I connect two nodes?',
      answer: 'Click and hold on an output anchor (the grey circle on the right side of a node) and drag the line to an input anchor (the circle on the left side of another node).'
    },
    {
      question: 'How do I delete a node or connection?',
      answer: 'To delete a node, right-click on it to open its configuration modal, where you will find a delete option. To delete a connection, click and drag one of its connection points away from its anchor and release it onto the empty canvas.'
    },
    {
      question: 'Why can’t I configure a Lexer or Parser node?',
      answer: 'You must first add and submit code using a "Source Code" node before you can configure the subsequent phases of the compiler.'
    }
  ];
