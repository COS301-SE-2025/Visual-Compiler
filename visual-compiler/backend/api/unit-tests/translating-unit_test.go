...
func TestReadRules_InvalidInput(t *testing.T) {
	...
	res, err := http.NewRequest("POST", "/api/translating/readRules", bytes.NewBuffer([]byte{}))
	if err != nil {
...
func TestTranslateCode_InvalidInput(t *testing.T) {
	...
	res, err := http.NewRequest("POST", "/api/translating/translate", bytes.NewBuffer([]byte{}))
	if err != nil {
		t.Errorf("Request could not be created")
	}
	res.Header.Set("Content-Type", "application/json")
